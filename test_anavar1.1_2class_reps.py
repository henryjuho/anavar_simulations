...
def read_sim_sfs(sfs_file):
    ...
    :return: list
    """
    sfs_line = [',\t'.join(x.split()) for x in open(sfs_file).readlines()[0].split('\n')[:-1]]
    return sfs_line
...
def main():
    ...
    sim_cmd = ('wolfram -script ~/anavar_simulations/snpdfe_spike_gen.v2.withopts_2class_reps.m ' +
               ' '.join([n, theta1, theta2, gamma1, gamma2, e1, e2, nreps, sfs_file]))
